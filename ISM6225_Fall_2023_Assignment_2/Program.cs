        ...
        public static bool IsValid(string s)
        {
            try
            {
                ...
                for (int i = 0; i < s.Length; i++)
                {
                    ...
                    {
                        if (bracketopen.Count == 0 || goodbracketcombos[bracketopen[bracketopen.Count - 1]] != c) // if count = 0 (there was no open bracket added to tracker
                                                                                                                  // OR the current character does not = the correct value
                                                                                                                  // (i.e.closing bracket) from the dictionary  
                        {
                            return false;
                        }
        ...
        public static int NumIdenticalPairs(int[] nums)
        {
            try
            {
                ...
                for (int i = 0; i < nums.Length; i++)
                {
                    ...
                    {
                        numbers[nums[i]] = 1; // if nums[i] not in dict, add it and give it a value of 1 
                    }
                }
